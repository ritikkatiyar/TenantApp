...
export default function SuperAdminLoginScreen({ onLogin, onNavigateToSignup }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      // For physical devices, we must use the host computer's local network IP
      ...
      if (!response.ok || !data.success) {
        setErrorMsg(data.error?.message || data.message || 'Login failed. Please check your credentials.');
        return;
      }
